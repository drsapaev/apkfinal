...
class LoginWithBiometricsUseCaseTest {
    ...
    @Test
    fun `invoke with null cipher returns failure without calling repository`() =
        runBlocking {
    ...
    @Test
    fun `invoke with blank phone returns failure without calling repository`() =
        runBlocking {
            val cipher = mockk<Cipher>(relaxed = true)
    ...
    @Test
    fun `invoke with valid inputs delegates to repository and returns success`() =
        runBlocking {
            val cipher = mockk<Cipher>(relaxed = true)
            coEvery { authRepository.loginWithBiometricRefreshToken(any(), any()) } returns
                Result.success(testUser)
    ...
    @Test
    fun `invoke trims phone before delegating`() =
        runBlocking {
            val cipher = mockk<Cipher>(relaxed = true)
            coEvery {
    ...
    @Test
    fun `invoke propagates repository failure on 401 expired session`() =
        runBlocking {
            val cipher = mockk<Cipher>(relaxed = true)
            coEvery { authRepository.loginWithBiometricRefreshToken(any(), any()) } returns
