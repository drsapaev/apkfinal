...
class NetworkBoundResourceTest {
    @Test
    fun `shouldFetch true - emits Loading then Success with updated data`() =
        runBlocking {
    ...
    @Test
    fun `shouldFetch false - emits Success with cached data only`() =
        runBlocking {
    ...
    @Test
    fun `network failure - emits Loading then Error with cached data`() =
        runBlocking {
            val query: () -> Flow<String> = { flowOf("cached-data") }
    ...
    @Test
    fun `onFetchFailed callback is called on network failure`() =
        runBlocking {
    ...
    @Test
    fun `saveFetchResult is called with network data on success`() =
        runBlocking {
    ...
    @Test
    fun `saveFetchResult is NOT called on network failure`() =
        runBlocking {
            val query: () -> Flow<String> = { flowOf("cache") }
    ...
    @Test
    fun `shouldFetch receives cached data for decision`() =
        runBlocking {
    ...
    @Test
    fun `empty cache triggers fetch, non-empty cache skips fetch`() =
        runBlocking {
    ...
    @Test
    fun `query is called at least twice - once for initial, once after save`() =
        runBlocking {
    ...
    @Test
    fun `error message falls back to 'Network error' when localizedMessage is null`() =
        runBlocking {
            val query: () -> Flow<String> = { flowOf("cache") }
