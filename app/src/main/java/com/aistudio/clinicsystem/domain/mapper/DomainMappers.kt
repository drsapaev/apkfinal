...
fun UserEntity.toDomain(): User = User(id = id.toString(), phone = phone, fullName = fullName, role = UserRole.fromBackend(role), dateOfBirth = dateOfBirth, biometricEnabled = biometricEnabled, telegramChatId = telegramChatId, clinicId = clinicId)
fun UserDto.toDomain(): User = User(id = id.toString(), phone = phone, fullName = fullName, role = UserRole.fromBackend(role), dateOfBirth = dateOfBirth, biometricEnabled = biometricEnabled, telegramChatId = telegramChatId, clinicId = clinicId)
fun UserProfileResponse.toDomain(): User = User(id = id.toString(), phone = phone ?: "", fullName = fullName ?: "", role = UserRole.fromBackend(role), dateOfBirth = dateOfBirth, biometricEnabled = biometricEnabled ?: false, telegramChatId = telegramChatId, clinicId = clinicId)
...
fun PendingSyncEntity.toDomain(): PendingSync = PendingSync(id = id, type = type, payload = payload, clientRequestId = clientRequestId, timestamp = timestamp, retryCount = retryCount, status = status, lastError = lastError, nextRetryAt = nextRetryAt)
