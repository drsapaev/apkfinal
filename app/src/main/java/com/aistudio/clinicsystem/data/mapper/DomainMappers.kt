package com.aistudio.clinicsystem.data.mapper
...
/**
 * Stage 5.2 (C-10 fix): Mappers moved from `domain/mapper/` to `data/mapper/`.
 *
 * The dependency direction is now correct: data layer knows about domain
 * layer (and can map between them), but domain layer does NOT know about
 * data layer. This closes audit finding C-10.
 *
 * These extension functions convert data-layer types (Room entities, Retrofit
 * DTOs) to domain-layer types (pure Kotlin data classes). The reverse
 * direction (domain → data) is not needed — the UI always works with domain
 * types, and writes go through repository methods that accept primitive
 * parameters, not domain objects.
 */
...
fun QueueSnapshotEntity.toDomain(): QueuePosition =
    QueuePosition(
        position = position,
        queueNumber = id,
...
fun PendingSyncEntity.toDomain(): PendingSync =
    PendingSync(
        id = id,
        type = type,
        payload = payload,
